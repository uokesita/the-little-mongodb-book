...
MapReduce es un enfoque al procesamiento de data que tiene dos grandes beneficios sobre los modelos tradicionales. La primera y principal raz�n es que fue desarrollado enfocado en el rendimiento. En teor�a MapReduce puede ser correr en paralelo permitiendo as� procesar largos conjuntos de data utilizando muchos procesadores o maquinas. Como ya mencionamos esto no es algo que MongoDB puede hacer actualmente. El segundo beneficio de MapReduce es que uno escribe c�digo para procesar la informaci�n. Comparado con lo que se hiciera en SQL, MapReduce es mucho mas rico y te deja aprovechar las ventajas del lenguaje de programaci�n que est�s usando. 

MapReduce es un patr�n que ha crecido en popularidad y que puedes usar en casi todos los lenguajes como: C#, Ruby, Java, Phyton, etc. Quiero advertirle que esto sonara muy complicado y diferente la primera vez que lo usa pero no se preocupe. Toma tu tiempo y juega con MapReduce. Vale la pena aprenderlo as� est�s usando MongoDB o no. 
...
El ejemplo que usaremos ser� generar un reporte del numero de hits por d�a que tenemos en una pagina. Esto es el "Hola Mundo" del MapReduce. Para esto vamos a usar una colecci�n de 'hits' con dos campos: recursos y fecha. El resultado que esperando este dividido por 'recurso', 'año', 'mes', 'd�a', 'veces'

Supongamos que tenemos la siguiente informaci�n: 
...
Esperar�amos el siguiente resultado:
...
(Lo bueno acerca este tipo de enfoque es que al ordenar el resultado, los reportes son r�pidos de generar y el crecimiento de la informaci�n esta controlado ya que solo usamos un documento de Mongo por cada recurso que queramos rastrear)

Por ahora solo debemos enfocarnos en entender los conceptos. Al final de este capitulo te proporcionaremos toda la data que usamos u el c�digo para que tu juegues por ti mismo. 

Lo primero que haremos es ver la funciona 'Map' o Mapa. El objetivo de esta funci�n es que retorne un valor que pueda ser reducido. Es posible que emita 0 o mas veces. En nuestro caso, siempre emitir� una sola vez (muy com�n). Imagina que Map es un bucle a trav�s de todos los hits. Por cada documento queremos generar una clave con un recurso, año, mes, d�a y valor. 
...
`this` se refiere al documento que estamos procesando actualmente. Vamos a ver el resultado de esta funci�n para entender un poco mas esto. Estamos usando la informaci�n que tenemos dos p�rrafos mas arriba. El resultado de la funci�n Map seria: 
...
El entendimiento de este paso intermediario es esencial para el entendimiento de MapReduce. Los valores que emite est�n agrupados como arreglos por clave. Los desarrolladores de .NET y Java pueden pensar de esto como un: `IDictionary<object, IList<object>>` (.NET) or `HashMap<Object, ArrayList>` (Java).


Cambiemos la funci�n de MAP un poco mas:
...
El primero resultado cambiar�a a:

	{resource: 'index', year: 2010, month: 0, day: 20} => [{count: 5}, {count: 1}, {count:1}]

Observa como cada `emit` genera un nuevo valor agrupado por nueva clave. 

La funci�n reduce o Reducir en español toma cada uno de estos valores intermedios y emite un resultado final. Nuestra funci�n para reducir es:
...
Como puedes notar este es el resultado que esper�bamos. 

Si has estado prestando atenci�n te preguntaras: *¿Por que no simplemente usamos `sum = values.lenght`?* Esto parece un enfoque eficiente cuando estas sumando arreglos de 1. La verdad es que en pocas ocasiones llamamos a la funci�n Reduce con un set perfecto. Por ejemplo en vez de ser llamado con: 
...
Al resultado final es el mismo (3) pero el camino tomado para llegar a el es diferente. Por lo tanto la funci�n reduce siempre debe ser indempotente. Esto es que la funci�n deber�a dar el mismo resultado as� se llame una sola vez o varias veces. 

Es muy com�n que encadenemos varias funciones Reduce cuando hacemos an�lisis complejos. 

### Pura Practica ###
Con MongoDB usamos el comando `mapReduce` en una colecci�n. `mapReduce` toma una funci�n para realizar el mapeo, una funciona para realizar la reducci�n y una directiva para el resultado. En nuestro terminal podemos pasarle una funci�n de Javascript. En muchas librer�as le daremos un `string` con nuestras funciones. Primero vamos a crear nuestro set de data para analizar:
...
Ahora podemos crear nuestras funciones Map y Reduce. El terminal de MongoDB acepta comandos multi linea y veras *..* despu�s de cotar enter para indicar que esperamos mas texto. 
...
Ahora podemos usar el comando `mapReduce` sobre la colecci�n de `hits`:

	db.hits.mapReduce(map, reduce, {out: {inline:1}})

Corriendo este comando podremos ver el resultado deseado. Cambiando `out` a `inline` significa que el resultado de `mapReduce` es inmediatamente transmitido a nosotros. Esto esta actualmente limitado a resultados que son menores de 16 megabytes. Pudi�ramos especificar `{out: 'hit_stats'}` y tener el resultado guardado en la colecci�n `hit_stats`

	db.hits.mapReduce(map, reduce, {out: 'hit_stats'});
	db.hit_stats.find();

Cuando realizamos esta acci�n perdemos toda la informaci�n almacenada en `hit_stats`. Si hici�ramos `{out: {merge: 'hi_stats'}}` las claves existentes ser�n remplazadas cuando los nuevos valores y los nuevas llaves ser� ensartadas como documentos. Finalmente podemos `out` usando una funci�n `reduce` para manejar casos mas avanzados. 

El tercer par�metro toma algunas opciones adicionales. Pudi�ramos filtrar, ordenar y limitar los documentos que queremos analizar. Tambi�n podemos especificar una funci�n `finalize` que ser� ejecutada contra el resultado despu�s de la funci�n `reduce`


### En Este Capitulo ###

Este es el primer capitulo donde cubrimos algo diferente. Si te hizo sentir incomodo, recuerda que siempre puedes usar otras funciones de MongoDB como la agregaci�n (http://www.mongodb.org/display/DOCS/Aggregation) para escenarios mas sencillos. Al final ten en cuanta que MapReduce es una de las mejores funcionalidad de MongoDB. La clave para entender como escribir las funciones de Map y Reduce es visualizar y entender la manera como la data intermedia luciera saliendo del `map` y entrando al `reduce`
